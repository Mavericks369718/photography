...
const CATEGORY_META = {
  wedding:     { base: 65000, durationLabel: "Full day · 10 hrs" },
  prewedding:  { base: 32000, durationLabel: "Half day · 6 hrs"  },
  cinematic:   { base: 55000, durationLabel: "Full day"          },
  maternity:   { base: 14999, durationLabel: "3 hrs"             },
  baby:        { base: 9999,  durationLabel: "2 hrs"             },
  fashion:     { base: 18999, durationLabel: "5 hrs"             },
  product:     { base: 14999, durationLabel: "Half day · 5 hrs"  },
  events:      { base: 12999, durationLabel: "4 hrs"             },
  corporate:   { base: 4999,  durationLabel: "2 hrs"             },
  realestate:  { base: 12999, durationLabel: "3 hrs"             }
};
...
export default function InstantBook() {
  ...
  const meta = CATEGORY_META[categoryId] || { base: 19999, durationLabel: "Half day" };
  const heroImg = cat?.image?.replace("w=400", "w=1400").replace("w=800", "w=1400") || "";
  const title = categoryTitle(categoryId);

  const [step, setStep] = useState(1); // 1 date · 2 time · 3 location · 4 service
  ...
  const [service, setService] = useState("photo");
  ...
  const total = Math.round(meta.base * svc.multiplier);
  const original = Math.round(total * 1.25);

  const canNext = (step === 1 && date) || (step === 2 && time) || (step === 3 && address.trim().length > 5) || (step === 4);

  const next = () => {
    if (step < 4) { setStep(step + 1); return; }
    // Finalize → write cm_booking & redirect to payment
    const booking = {
      package: {
      ...
      <div className="relative w-full max-w-[480px] min-h-screen bg-[#FFF1EC] flex flex-col pb-28">
        ...
        <div className="relative h-[34vh] min-h-[260px] w-full overflow-hidden">
          ...
          <div className="absolute top-0 left-0 right-0 z-10 px-5 pt-6 flex items-center justify-between">
            ...
            <span className="inline-flex items-center gap-1.5 rounded-full bg-white/15 backdrop-blur ring-1 ring-white/25 px-2.5 py-1 text-[10px] font-semibold tracking-[0.22em] uppercase text-white">Step {step} / 4</span>
          </div>
          <div className="absolute z-10 bottom-0 left-0 right-0 px-6 pb-7">
            ...
            <h1 className="mt-1.5 text-white text-[30px] leading-[1.02] tracking-[-0.025em] drop-shadow-[0_2px_10px_rgba(0,0,0,0.4)]" style={{fontWeight:800}}>
              {step === 1 && <>Pick a <span style={{fontFamily:"'Fraunces',serif",fontWeight:500,fontStyle:"italic"}} className="text-rose-200">date.</span></>}
              {step === 2 && <>Choose a <span style={{fontFamily:"'Fraunces',serif",fontWeight:500,fontStyle:"italic"}} className="text-rose-200">time.</span></>}
              {step === 3 && <>Where to <span style={{fontFamily:"'Fraunces',serif",fontWeight:500,fontStyle:"italic"}} className="text-rose-200">shoot?</span></>}
              {step === 4 && <>What do you <span style={{fontFamily:"'Fraunces',serif",fontWeight:500,fontStyle:"italic"}} className="text-rose-200">need?</span></>}
            </h1>
          </div>
        </div>
        ...
        <div className="relative -mt-6 flex-1 bg-white rounded-t-[28px] px-5 pt-5 pb-6 shadow-[0_-10px_30px_rgba(0,0,0,0.08)]">
          ...
          <div className="flex items-center gap-1.5 mb-5">
            ...
            <span className="ml-2 text-[11px] font-semibold tracking-[0.18em] uppercase text-neutral-500">
              {step===1?"Date":step===2?"Time":step===3?"Location":"Service"}
            </span>
          </div>

          {step === 1 && (
            <div data-testid="ib-step-date">
              <div className="rounded-2xl bg-white ring-1 ring-neutral-200 shadow-[0_6px_18px_rgba(0,0,0,0.04)] p-4">
                ...
                <div className="grid grid-cols-7 gap-1">
                  {cells.map((d,i)=>{
                    ...
                    return (
                      <button key={i} data-testid={sel?"ib-date-selected":undefined} onClick={()=>!dis && setDate(d)} disabled={dis}
          ...
          {step === 2 && (
            <div data-testid="ib-step-time">
              <p className="text-[11px] font-semibold tracking-[0.22em] uppercase text-rose-700 mb-2.5">Preferred start</p>
              <div className="grid grid-cols-3 gap-2">
                {TIME_SLOTS.map(t=>{
                  const sel = time === t;
                  return (
                    <button key={t} data-testid={`ib-time-${t.replace(/[: ]/g,'')}-btn`} onClick={()=>setTime(t)}
                      className={`py-3 rounded-xl text-[13px] font-semibold ring-1 transition ${sel?"bg-rose-800 text-white ring-rose-800 shadow-md":"bg-white text-neutral-800 ring-neutral-200 hover:ring-rose-300"}`}>
                      {t}
                    </button>
                  );
                })}
              ...
              {date && (
                <p className="mt-5 text-[12.5px] text-neutral-500 inline-flex items-center gap-1.5">
                  <Clock className="h-3.5 w-3.5"/> Booking for {date.toLocaleDateString("en-IN", { weekday:"short", day:"2-digit", month:"short" })}
                </p>
              )}
            </div>
          )}
          ...
          {step === 3 && (
            <div data-testid="ib-step-location">
              ...
              <div className="relative h-[200px] rounded-2xl overflow-hidden ring-1 ring-neutral-200">
                ...
                <button
                  onClick={()=>{ setAddress(RECENT_ADDRESSES[0].line); setPinLabel("Detected · " + RECENT_ADDRESSES[0].label); }}
                  data-testid="ib-detect-location-btn"
          ...
          {step === 4 && (
            <div data-testid="ib-step-service">
              ...
              <p className="mt-5 text-[11.5px] text-neutral-500 leading-relaxed">A photographer will be auto-assigned based on your location & expertise match.</p>
            </div>
          )}
        </div>
        ...
        <div className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-[480px] bg-white/95 backdrop-blur border-t border-neutral-200 px-5 py-3.5 z-40">
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <span className="text-[11px] font-semibold text-emerald-700">{meta.durationLabel}</span>
            </div>
            <button data-testid="ib-next-btn" onClick={next} disabled={!canNext}
              ...
              {step === 4 ? "Continue to pay" : "Continue"} <ArrowRight className="h-4 w-4"/>
            </button>
          </div>
        </div>
      </div>
